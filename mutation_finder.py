...
mutation_finder_output_file_extension = 'txt'
...
class PointMutation(Mutation):
    ...
    def _get_original_mention(self):
        return self.__original_mention
...
class ChrPointMutation(Mutation):
    """A class for storing information about point mutations in chromosome
        format
    """

    def __init__(self, position, wt_nucleotide, mut_nucleotide, originalMention):
      self.__wt_nucleotide = wt_nucleotide
      self.__mut_nucleotide = mut_nucleotide
      self.__original_mention = originalMention
      Mutation.__init__(self, position)

    def _get_wt_nucleotide(self):
      return self.__wt_nucleotide
    WtNucleotide = property(_get_wt_nucleotide)

    def _get_mut_nucleotide(self):
      return self.__mut_nucleotide
    MutNucleotide = property(_get_mut_nucleotide)
    ...
    def __str__(self):
      return ''.join(['c.', str(self.Position), self.WtNucleotide, '>', self.MutNucleotide])
    ...
    def __eq__(self,other):
        ...
        if type(self) == type(other):
          return self.Position == other.Position and \
                 self.__wt_nucleotide== other.__wt_nucleotide and \
                 self.__mut_nucleotide == other.__mut_nucleotide
        return False
...
class MutationFinder(MutationExtractor):
    ...
    def _post_process(self,mutations):
        ...
        for mutation in mutations.keys():
            ...
            elif type(mutation) is ChrPointMutation:
              if mutation.WtNucleotide == mutation.MutNucleotide:
                  del mutations[mutation]
    ...
    def __call__(self,raw_text):
        ...
        for regular_expression in self._regular_expressions:
            for m in regular_expression.finditer(raw_text):
                # Taking care of whether or not the mutation found is
                # in chromosome notation or protein notation
                try:
                  ...
                  current_mutation = \
                    ChrPointMutation(m.group('pos'), m.group('wt_nuc'),\
                              m.group('mut_nuc'), raw_text[span[0]:span[1]+1])

                # Currently hardcoded to 100 but should in the future be
                # a command line parameter?
                surrounding_text = raw_text[max(span[0]-100, 0):\
                                      min(len(raw_text)-1, span[1]+100)]
                #if raw_text[span[0]-2].lowercase() == 'c':
...
def parse_command_line_parameters():
    ...
    parser.set_defaults(verbose=False,output_dir='./',\
        use_baseline_system=False,store_spans=True,
...
def extract_mutations_from_lines_to_file(lines,output_filepath,\
   mutation_extractor,store_spans=True,output_normalized_mutations=False):
    """
    ...
    for identifier,mutations in \
        ...
        if output_normalized_mutations:
            ...
            # to a string containing the 200 character span around each mention. For
            ...
            for mutation,value in mutations.items():
                if store_spans:
                    ...
                    # Write the current mutation and its spans
                        value = value.replace('\t', ' ')
                        mutation_entry = str(mutation) + ':' + value + ':' + mutation.OriginalMention
                        output_fields += [mutation_entry]
